...
#define NUM_ELEM ((SIZEINMB * 1024 * 1024) / (sizeof(unsigned int)))
...
static unsigned int mem[NUM_ELEM];
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < nsecs; i++) {
    for (j = 0; j < 200; j++) {
      mem[rnd() % NUM_ELEM] ^= rnd();
    }
    sleep(1);
  }

  for (v = 0, i = 0; i < NUM_ELEM; i += 512) {
    v ^= mem[i];
  }
...
void setseed (int v) {
  _x = v;
  /*
   * Call rnd() twice to get a better seed value
   * - first one is often not so good.
   */
  rnd();
  rnd();
}
